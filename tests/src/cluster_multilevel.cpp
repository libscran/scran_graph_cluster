...
TEST(ClusterMultilevel, Basic) {
    ...
    auto output = scran::cluster_multilevel::compute(mock.first, mock.second, scran::cluster_multilevel::Options());
...
TEST(ClusterMultilevel, Parameters) {
    ...
    scran::cluster_multilevel::Options opts;
    auto output1 = scran::cluster_multilevel::compute(mock.first, mock.second, opts);

    // Responds to changes.
    {
        scran::cluster_multilevel::Options opts;
        opts.resolution = 2;
        auto output2 = scran::cluster_multilevel::compute(mock.first, mock.second, opts);
    ...
    // We can disable reporting.
    {
        ...
        auto output = scran::cluster_multilevel::compute(mock.first, mock.second, opts);
